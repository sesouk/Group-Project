        ...
        {client_id: process.env.REACT_APP_AUTH0_CLIENT_ID,
          ...
          .then( userInfoResponse => {
            // console.log('--------------------userInfoResponse', userInfoResponse.data)
              ...
              // const location = req.session.user.lastLocation
              // console.log('last location is', req.session.user)
                // console.log('-------- userData', userData );
                req.session.user.name = userData.name
                req.session.user.email = userData.email
                req.session.user.cart[0] ? req.session.user.cart = req.session.user.cart : []
